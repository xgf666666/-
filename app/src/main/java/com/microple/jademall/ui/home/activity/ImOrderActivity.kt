...
//            }
        var str=""
        for (i in 0 ..buyTyle.buy_type.size-1){
            if (i==0){
                str="${buyTyle.buy_type[i].bt_id}"
            }else{
...
//                            showToast("支付成功")
    ...
    lateinit var goods:ImOrder.OrderBean.GoodsInfoBean
    override fun initData() {
        ...
        adapter.setOnItemChildClickListener { adapter, view, position ->
            goods = (adapter as ImOrderAdapter).data[position]
            if (view.id == R.id.ll_type) {
                getPresenter().getBuyTyle(Constants.getToken(), "" + goods.type)
            }
        }
    }
